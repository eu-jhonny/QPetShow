      ...
      <div className="grid gap-8 lg:grid-cols-[260px_1fr]">
        <aside className="h-fit rounded-3xl border border-black/5 bg-white p-5 shadow-soft lg:sticky lg:top-40 dark:border-white/10 dark:bg-white/5">
          ...
          <nav aria-label="Menu da conta" className="flex flex-col gap-1">
            <AccountNavLink href="/conta" label="Meu perfil" icon="user" exact />
            <AccountNavLink href="/conta/pedidos" label="Meus pedidos" icon="package" />
            <AccountNavLink href="/conta/favoritos" label="Favoritos" icon="heart" />
            <LogoutButton />
          </nav>
        </aside>
        <div>{children}</div>
