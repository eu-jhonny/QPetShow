...
import { z } from "zod";
import { getOrder, updateOrderStatus, updateOrderFields, type OrderStatus } from "@/lib/server/orders";
import { getAdminSession } from "@/lib/server/auth";
import { sendPaymentApproved, sendOrderShipped, sendOrderConfirmation } from "@/lib/email";
...
const postSchema = z.object({
  action: z.enum(["resend", "note"]),
  note: z.string().trim().max(500).optional(),
});

/** Ações extras: reenviar e-mail de confirmação ou salvar nota interna. */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (parsed.data.action === "note") {
    const updated = await updateOrderFields(id, { adminNote: parsed.data.note ?? "" });
  ...
  const result = await sendOrderConfirmation(order.customer.email, lite);
